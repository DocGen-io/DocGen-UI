...
export function RevisionsPage() {
  ...
  const filteredRevisions =
    revisions?.filter((r) => (filter === "all" ? true : r.status === filter)) ||
    [];
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      ...
      {isLoading ? (
        ...
        <Tabs
          value={filter}
          onValueChange={(v) => setFilter(v as RevisionStatus | "all")}
          className="w-full"
        >
          <TabsList className="bg-muted/50 p-1">
            ...
            >
              All{" "}
              <span className="ml-2 px-1.5 py-0.5 rounded-full bg-muted text-[10px]">
                {counts.all}
              </span>
            </TabsTrigger>
            <TabsTrigger
            ...
            >
              Pending{" "}
              <span className="ml-2 px-1.5 py-0.5 rounded-full bg-orange-500/10 text-orange-600 text-[10px]">
                {counts.pending}
              </span>
            </TabsTrigger>
            <TabsTrigger
            ...
            >
              Approved{" "}
              <span className="ml-2 px-1.5 py-0.5 rounded-full bg-green-500/10 text-green-600 text-[10px]">
                {counts.approved}
              </span>
            </TabsTrigger>
            <TabsTrigger
            ...
            >
              Rejected{" "}
              <span className="ml-2 px-1.5 py-0.5 rounded-full bg-rose-500/10 text-rose-600 text-[10px]">
                {counts.rejected}
              </span>
            </TabsTrigger>
          </TabsList>
              ...
              <div className="grid grid-cols-1 gap-6">
                {filteredRevisions.map((revision) => (
                  <RevisionCard
                    ...
                    isProcessing={
                      approveRevision.isPending || rejectRevision.isPending
                    }
                  />
                ))}
              </div>
