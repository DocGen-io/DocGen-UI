...
export function ConfigPage() {
  ...
  const { data: fetchedConfigWrapper, isLoading } = useTeamConfig(activeTeam?.id);
  
  const fetchedConfig = fetchedConfigWrapper?.config_data || fetchedConfigWrapper;
  ...
  if (!activeTeam) {
    return (
      <div className="p-6">
        <Alert>
          <AlertDescription>Please select a team to view configuration</AlertDescription>
        </Alert>
      </div>
    );
  }
