...
import { Save, RotateCcw, CheckCircle2, AlertTriangle, FileCode } from 'lucide-react';
...
export function OpenAPIEditor({ initialValue, onSave, title }: OpenAPIEditorProps) {
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold tracking-tight">{title} Editor</h3>
        </div>
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={handleSave} disabled={!isValid} className="h-8 bg-primary">
      ...
      <div className="relative group">
        <textarea
          ...
          className={`w-full h-[60vh] p-6 font-mono text-[13px] bg-muted/30 rounded-2xl border ${
            isValid ? 'border-border focus:border-primary/50' : 'border-destructive focus:border-destructive'
        ...
        <div className="absolute bottom-4 right-6 flex items-center gap-3">
          {isValid ? (
            <Badge variant="outline" className="bg-green-500/10 text-green-500 border-green-500/20 py-1 flex items-center gap-1.5">
              <CheckCircle2 className="w-3 h-3" />
              Valid OpenAPI JSON
            </Badge>
          ) : (
            <Badge variant="outline" className="bg-red-500/10 text-red-500 border-red-500/20 py-1 flex items-center gap-1.5">
