...
export function ProjectSelector({ projectName, setProjectName, availableProjects, setSearchParams }: ProjectSelectorProps) {
  if (!availableProjects || availableProjects.length === 0) return null;

  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest hidden sm:inline">Active Project:</span>
      <Select value={projectName} onValueChange={(val) => {
        ...
        <SelectContent className="bg-card/95 backdrop-blur-xl border-primary/10 rounded-xl">
          {availableProjects.map(p => (
            <SelectItem key={p} value={p} className="capitalize py-2.5">{p.replace(/-/g, ' ')}</SelectItem>
          ))}
        </SelectContent>
      </Select>
    </div>
