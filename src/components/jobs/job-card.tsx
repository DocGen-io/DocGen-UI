...
import JobMetaDetails from "@/components/jobs/job-meta-details";
import JobActionsMenu from "@/components/jobs/job-action-menu";
import JobStatusMessage from "@/components/jobs/job-status-message";
...
export function JobCard({ job, onClick, onDelete }: JobCardProps) {
  return (
    <div className="cursor-pointer group" onClick={() => onClick?.(job.id)}>
